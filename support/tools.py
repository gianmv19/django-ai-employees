...
def get_order_details(order_id):
	try:
		...
		return {
			...
			"ordered_on": order.created_at.strftime("%d %b %Y"), # 25 Jun 2026
			"days_since_order": (timezone.now() - order.created_at).days, #25 Days
		}
...
def get_refund_history(user_id):
	...
	for refund in refunds:
		history.append({
			...
			"requested_on": refund.created_at.strftime("%d %b %Y"), #26 Jun 2026
		})
	return {
...
def check_delivery_status(tracking_number, carrier):
	default_response = {
		"status": "Unknown",
	    "last_location": "Tacking info unavailable",
	    "last_update": "N/A",
